...
def run_update(
    *,
    ...
    if skill_name:
        local_skills = [(d, u) for d, u in local_skills if d.name == skill_name]
        if not local_skills:
            typer.echo(
    ...
    for skill_dir, repo_url in local_skills:
        by_repo[repo_url].append(skill_dir)

    # 4. Build update plan (before touching any files)
    update_plan: list[dict] = []  # {"skill_dir": Path, "repo_url": str, "name": str}
    not_found: list[dict] = []

    for repo_url, skill_dirs in by_repo.items():
        typer.echo(f"\n⏳ Checking {repo_url} …")
        try:
                        ...
                        update_plan.append(
                            {
                                ...
                                "skill_dir": skill_dir,
                                "repo_url": repo_url,
                                "remote_source": remote_by_name[name].source_path,
                            }
                        )
                    else:
            ...
            for skill_dir in skill_dirs:
                not_found.append({"name": skill_dir.name, "repo_url": repo_url})
            continue
    ...
    if not skill_name:
        summary = _build_update_summary(update_plan, not_found)
        ...
        try:
            with cloned_repo(repo_url) as repo_path:
                ...
                for item in items:
                    ...
                    copy_skill(remote.source_path, local_dir.parent, repo_url)
...
def _build_update_summary(update_plan: list[dict], not_found: list[dict]) -> str:
    lines = [
        "",
        "┌─────────────────────────────────────────────",
        "│  Update Summary",
